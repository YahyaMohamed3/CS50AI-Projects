...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap_x_y is not None:
            i, j = overlap_x_y
            revised = False
            for word_x in list(self.domains[x]):  # Iterate over a copy of the set
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        def num_conflicting_values(value):
            return sum(1 for neighbor in self.crossword.neighbors(var)
            if neighbor in assignment and value in self.domains[neighbor]
            and any(value[i] != assignment[neighbor][j] for i, j in [self.crossword.overlaps[var, neighbor]]))


        return sorted(self.domains[var], key=num_conflicting_values)
